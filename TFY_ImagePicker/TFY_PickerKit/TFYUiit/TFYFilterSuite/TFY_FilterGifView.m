...
inline static NSTimeInterval TFYFilterGifView_CGImageSourceGetGifFrameDelay(CGImageSourceRef imageSource, NSUInteger index)
{
...
@implementation TFY_FilterGifView
        ...
        if (gifData) {
            ...
            if (_frameCount) {
                ...
                while (index < _frameCount) {
                    [durations addObject:@(TFYFilterGifView_CGImageSourceGetGifFrameDelay(_gifSourceRef, index))];
                    index ++;
                }
                _durations = [durations copy];
            }
...
- (void)setupDisplayLink {
    ...
    if (_displayLink == nil && _frameCount > 1) {
        TFY_WeakSelectorTarget *target = [[TFY_WeakSelectorTarget alloc] initWithTarget:self targetSelector:@selector(displayGif)];
