...
{
    TFY_AudioItem *item = [self new];
    if (item) {
        item.title = [NSBundle picker_localizedStringForKey:@"_LFME_defaultAudioItem_name"];
        item.isEnable = YES;
    }
    return item;
}
...
- (BOOL)isOriginal
{
    return self.title == [NSBundle picker_localizedStringForKey:@"_LFME_defaultAudioItem_name"] && self.url == nil;
}

@end

@interface TFY_AudioTrackCell : UITableViewCell
@end

@implementation TFY_AudioTrackCell
...
@interface TFY_AudioTrackBar ()<UITableViewDelegate, UITableViewDataSource, MPMediaPickerControllerDelegate>

@property (nonatomic, strong) NSMutableArray <NSMutableArray <TFY_AudioItem *> *> *m_audioUrls;
...
@implementation TFY_AudioTrackBar
...
- (void)configCustomNaviBar
{
    ...
    UIView *topbar = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.picker_width, _customTopbarHeight)];
    ...
    UIButton *cancelButton = [[UIButton alloc] initWithFrame:CGRectMake(margin, topbar.picker_height-size, editCancelWidth, size)];
    ...
    UIButton *finishButton = [[UIButton alloc] initWithFrame:CGRectMake(self.picker_width - editOkWidth - margin, topbar.picker_height-size, editOkWidth, size)];
...
{
    UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, _customTopbarHeight, self.picker_width, self.picker_height-_customTopbarHeight-_customToolbarHeight) style:UITableViewStylePlain];
...
- (void)configToolbar
{
    UIView *toolbar = [[UIView alloc] initWithFrame:CGRectMake(0, self.picker_height-_customToolbarHeight, self.picker_width, _customToolbarHeight)];
...
{
    if ([self.delegate respondsToSelector:@selector(picker_audioTrackBarDidCancel:)]) {
        [self.delegate picker_audioTrackBarDidCancel:self];
    }
}

- (void)finishButtonClick
{
    if ([self.delegate respondsToSelector:@selector(picker_audioTrackBar:didFinishAudioUrls:)]) {
        [self.delegate picker_audioTrackBar:self didFinishAudioUrls:self.audioUrls];
    }
}

- (void)audioTrackTrash
{
    NSMutableArray <TFY_AudioItem *>*deleteItems = [@[] mutableCopy];
    for (NSMutableArray *list in self.m_audioUrls) {
        for (TFY_AudioItem *item in list) {
...
- (void)audioTrackAdd
{
    ...
    picker.prompt = [NSBundle picker_localizedStringForKey:@"_LFME_MediaPicker_prompt"];   //弹出选择播放歌曲的提示
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TFY_AudioItem *item = list[indexPath.row];
    item.isEnable = !item.isEnable;
...
- (nullable UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    label.text = [NSBundle picker_localizedStringForKey:@"_LFME_AudioTackTitle_prompt"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TFY_AudioTrackCell *cell = [tableView dequeueReusableCellWithIdentifier:audioTrackCellIdentifier];
    if (cell == nil) {
        cell = [[TFY_AudioTrackCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:audioTrackCellIdentifier];
    }
    NSMutableArray *list = self.m_audioUrls[indexPath.section];
    TFY_AudioItem *item = list[indexPath.row];
...
- (NSArray<TFY_AudioItem *> *)audioUrls
{
