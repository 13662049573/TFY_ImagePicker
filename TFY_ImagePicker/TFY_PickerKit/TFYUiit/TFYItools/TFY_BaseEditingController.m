...
@interface TFY_BaseEditingController ()
{
...
@implementation TFY_BaseEditingController
...
- (instancetype)initWithOrientation:(UIInterfaceOrientation)orientation
{
    ...
    if (self) {
        ...
        [UIDevice picker_setOrientation:orientation];
        ...
        /** 创建笔刷缓存 */
        [TFY_BrushCache share].countLimit = 20;
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (@available(iOS 13.0, *)) {
        if (isiPhone && self.navigationController.modalPresentationStyle == UIModalPresentationPageSheet) {
            ...
            // 彻底关闭下拉手势
            self.picker_dropShadowPanGestureRecognizer.enabled = NO;
        }
    }
}
...
{
    TFY_EasyNoticeBarConfig *config = [TFY_EasyNoticeBarConfig new];
    config.title = text;
    config.type = TFYEasyNoticeBarDisplayTypeInfo;
    [TFY_EasyNoticeBar showAnimationWithConfig:config];
}
...
- (void)showErrorMessage:(NSString *)text
{
    TFY_EasyNoticeBarConfig *config = [TFY_EasyNoticeBarConfig new];
    config.title = text;
    config.type = TFYEasyNoticeBarDisplayTypeError;
    [TFY_EasyNoticeBar showAnimationWithConfig:config];
}
...
- (void)showProgressHUDText:(NSString *)text isTop:(BOOL)isTop needProcess:(BOOL)needProcess
{
    ...
    _HUDLabel.text = text ? text : [NSBundle picker_localizedStringForKey:@"_LFME_processHintStr"];
