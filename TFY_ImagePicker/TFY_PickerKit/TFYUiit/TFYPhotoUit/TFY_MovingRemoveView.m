...
@implementation TFY_MovingRemoveView
...
- (void)layoutSubviews
{
    ...
    CGFloat margin = 10.0;
    [self picker_setCornerRadius:self.bounds.size.width*0.1];
...
- (void)setSelected:(BOOL)selected
{
    if (_selected != selected) {
        ...
        if (selected) {
            ...
            self.label.text = [NSBundle picker_localizedStringForKey:@"_LFME_sticker_remove_selected"];
        } else {
            self.contentView.backgroundColor = self.normailColor;
            self.imageView.image = bundleEditImageNamed(@"EditImageStickRemove.png");
            self.label.text = [NSBundle picker_localizedStringForKey:@"_LFME_sticker_remove_normal"];
        }
    }
}
