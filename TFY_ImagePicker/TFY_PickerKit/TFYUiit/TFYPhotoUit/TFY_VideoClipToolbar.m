...
@implementation TFY_VideoClipToolbar
...
{
    if ([self.delegate respondsToSelector:@selector(picker_videoClipToolbarDidCancel:)]) {
        [self.delegate picker_videoClipToolbarDidCancel:self];
    }
}

- (void)clippingOk:(UIButton *)button
{
    if ([self.delegate respondsToSelector:@selector(picker_videoClipToolbarDidFinish:)]) {
        [self.delegate picker_videoClipToolbarDidFinish:self];
    }
}
