...
inline static NSTimeInterval TFYMEGifView_CGImageSourceGetGifFrameDelay(CGImageSourceRef imageSource, NSUInteger index)
{
...
inline static UIImageOrientation TFYMEGifView_UIImageOrientationFromEXIFValue(NSInteger value) {
    switch (value) {
        ...
        if (image) {
            ...
            if (_image.images.count > 1) {
                ...
                dispatch_async(self.serialQueue, ^{
                    CGImageRef decodeImageRef = picker_CGImageScaleDecodedFromCopy(image.CGImage, size, mode, orientation);
    ...
    if (_image == nil) {
        ...
        if (self.data) {
            if (_frameCount > 1) {
                ...
                CGImageRef decodeImageRef = picker_CGImageDecodedFromCopy(imageRef);
                if (imageRef) {
                    CGImageRelease(imageRef);
                }
    ...
    if (_data != data) {
        ...
        if (data) {
            ...
            if(CFDictionaryContainsKey(exifInfo, kCGImagePropertyOrientation)){
                ...
                _orientation = TFYMEGifView_UIImageOrientationFromEXIFValue(orientationValue);
            }
            CFRelease(exifInfo);
            ...
            if (_frameCount > 1) {
                ...
                while (index < _frameCount) {
                    [durations addObject:@(TFYMEGifView_CGImageSourceGetGifFrameDelay(_gifSourceRef, index))];
                    index ++;
                }
                ...
                dispatch_async(self.serialQueue, ^{
                    ...
                    CGImageRef decodeImageRef = picker_CGImageScaleDecodedFromCopy(imageRef, size, mode, orientation);
                    if (imageRef) {
                        CGImageRelease(imageRef);
                    }
                    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)setupDisplayLink {
    if (_displayLink == nil && _frameCount > 1) {
        TFY_MEWeakSelectorTarget *target = [[TFY_MEWeakSelectorTarget alloc] initWithTarget:self targetSelector:@selector(displayGif)];
...
- (void)displayGif
{
    ...
    while (_timestamp >= [self frameDurationAtIndex:_index]) {
        ...
        if (imageRef == NULL) {
            ...
            if (imageRef) {
                CGImageRef decodeImageRef = picker_CGImageScaleDecodedFromCopy(imageRef, self.frame.size, self.contentMode, self.orientation);
