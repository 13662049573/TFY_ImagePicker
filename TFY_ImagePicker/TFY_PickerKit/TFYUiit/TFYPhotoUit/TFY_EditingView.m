...
CGFloat const picker_editingView_drawLineWidth = 5.f;
CGFloat const picker_editingView_splashWidth = 25.f;

typedef NS_ENUM(NSUInteger, TFYEditingViewOperation) {
    TFYEditingViewOperationNone = 0,
    TFYEditingViewOperationDragging = 1 << 0,
    TFYEditingViewOperationZooming = 1 << 1,
    TFYEditingViewOperationGridResizing = 1 << 2,
};

NSString *const kTFYEditingViewData = @"TFYClippingViewData";

NSString *const kTFYEditingViewData_gridView_aspectRatio = @"kTFYEditingViewData_gridView_aspectRatio";

NSString *const kTFYEditingViewData_clippingView = @"kTFYEditingViewData_clippingView";

@interface TFY_EditingView ()<UIScrollViewDelegate, TFYClippingViewDelegate, TFYGridViewDelegate>
...
@property (nonatomic, copy) picker_me_dispatch_cancelable_block_t maskViewBlock;

/** 编辑操作次数记录-有3种编辑操作 拖动、缩放、网格 并且可以同时触发任意2种，避免多次回调代理 */
@property (nonatomic, assign) TFYEditingViewOperation editedOperation;
...
/** 记录剪裁前的数据 */
@property (nonatomic, assign) TFYGridViewAspectRatioType old_aspectRatio;
...
@implementation TFY_EditingView
...
- (void)customInit
{
    ...
    /** 创建剪裁层 */
    TFY_ClippingView *clippingView = [[TFY_ClippingView alloc] initWithFrame:self.bounds];
    clippingView.clippingDelegate = self;
    [self.clipZoomView addSubview:clippingView];
    self.clippingView = clippingView;
    
    TFY_GridView *gridView = [[TFY_GridView alloc] initWithFrame:self.bounds];
    ...
    /** 创建显示图片像素控件 */
    UILabel *imagePixel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.picker_width-40, 30)];
...
- (void)setImage:(UIImage *)image durations:(NSArray <NSNumber *> *)durations
{
    ...
    if (image) {
        ...
        /** 参数取整，否则可能会出现1像素偏差 */
        cropRect = TFYMediaEditProundRect(cropRect);
    ...
    /** 计算图片像素参照坐标 */
    self.referenceSize = AVMakeRectWithAspectRatioInsideRect(self.clippingView.picker_size, self.clippingMaxRect).size;
...
- (void)setClippingMinSize:(CGSize)clippingMinSize
{
    if (CGSizeEqualToSize(CGSizeZero, _clippingMinSize) || (clippingMinSize.width < CGRectGetWidth(_clippingMaxRect) && clippingMinSize.height < CGRectGetHeight(_clippingMaxRect))) {
        ...
        CGSize normalClippingMinSize = AVMakeRectWithAspectRatioInsideRect(self.clippingView.picker_size, [self refer_clippingRect]).size;
        /** 需要考虑到旋转后到尺寸可能会更加小，取最小值 */
        CGSize rotateClippingMinSize = AVMakeRectWithAspectRatioInsideRect(CGSizeMake(self.clippingView.picker_size.height, self.clippingView.picker_size.width), [self refer_clippingRect]).size;
...
- (void)setClipping:(BOOL)clipping animated:(BOOL)animated
{
    ...
    self.editedOperation = TFYEditingViewOperationNone;
    _clipping = clipping;
    ...
    if (clipping) {
        if ([self.clippingDelegate respondsToSelector:@selector(picker_EditingViewWillAppearClip:)]) {
            [self.clippingDelegate picker_EditingViewWillAppearClip:self];
        }
    } else {
        if ([self.clippingDelegate respondsToSelector:@selector(picker_EditingViewWillDisappearClip:)]) {
            [self.clippingDelegate picker_EditingViewWillDisappearClip:self];
        }
    }
    ...
    if (clipping) {
        ...
        if (animated) {
            ...
            [UIView animateWithDuration:0.25f animations:^{
                self.clippingRect = AVMakeRectWithAspectRatioInsideRect(self.clippingView.picker_size, [self refer_clippingRect]);
                ...
                [UIView animateWithDuration:0.25f animations:^{
                    ...
                    if (self.onceDefaultAspectRatioIndex) {
                        ...
                        if ([self.clippingDelegate respondsToSelector:@selector(picker_EditingViewDidAppearClip:)]) {
                            [self.clippingDelegate picker_EditingViewDidAppearClip:self];
                        }
                        [self.gridView setAspectRatio:self.onceDefaultAspectRatioIndex animated:YES];
                        self.onceDefaultAspectRatioIndex = 0;
                    } else {
                        /** 处理缩放比例 */
                        [self picker_gridViewDidAspectRatio:self.gridView];
        ...
        if (animated) {
            ...
            [UIView animateWithDuration:0.1f animations:^{
                ...
                [UIView animateWithDuration:0.125f delay:0.125f options:UIViewAnimationOptionCurveEaseOut animations:^{
                    ...
                    if ([self.clippingDelegate respondsToSelector:@selector(picker_EditingViewDidDisappearClip:)]) {
                        [self.clippingDelegate picker_EditingViewDidDisappearClip:self];
                    }
                }];
...
- (void)cancel
{
    ...
    _clippingRect = AVMakeRectWithAspectRatioInsideRect(self.clippingView.picker_size, self.bounds);
...
/** 重写比例配置 */
- (void)setExtraAspectRatioList:(NSArray<id<TFY_ExtraAspectRatioProtocol>> *)extraAspectRatioList
{
    self.gridView.extraAspectRatioList = extraAspectRatioList;
}

- (NSArray<id<TFY_ExtraAspectRatioProtocol>> *)extraAspectRatioList
{
...
- (void)setAspectRatioIndex:(NSUInteger)aspectRatioIndex
{
    ...
    self.onceDefaultAspectRatioIndex = 0;
    [self.gridView setAspectRatio:(TFYGridViewAspectRatioType)aspectRatioIndex];
}
...
- (NSUInteger)aspectRatioIndex
{
    ...
    TFYGridViewAspectRatioType type = self.gridView.aspectRatio;
...
- (void)supplementHandle
{
    if (!CGRectEqualToRect(self.gridView.gridRect, self.clippingView.frame)) {
        self.gridView.showMaskLayer = NO;
        picker_me_dispatch_cancel(self.maskViewBlock);
...
//    CGSize contentSize = self.clippingView.contentSize;
    ...
    /** 参数取整，否则可能会出现1像素偏差 */
    clippingRect = TFYMediaEditProundRect(clippingRect);
...
//    CGFloat degree = rotate/M_PI * 180;
    ...
    /** 参数取整，否则可能会出现1像素偏差 */
    clipRect = TFYMediaEditProundRect(clipRect);
...
//    clipRect.origin.y = (int)((contentSize.height/clipScale - clipRect.size.height - clipRect.origin.y)+0.5)*1.f;
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        UIImage *(^ClipEditImage)(UIImage *) = ^UIImage * (UIImage *image) {
            ...
            @autoreleasepool {
                /** 剪裁图片 */
                returnedImage = [image picker_cropInRect:clipRect];
                if (rotate > 0) {
                    /** 调整角度 */
                ...
                if (otherImage) {
                    ...
                    if (scaleOtherImage) {
                        /** 合并图层 */
                        returnedImage = [returnedImage picker_mergeimages:@[scaleOtherImage]];
                    }
                }
            }
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (!resultImage) {
                /** 合并操作有误，直接截取原始图层 */
                resultImage = [self.clipZoomView picker_captureImageAtFrame:self.clippingView.frame];
            }
...
#pragma mark - TFYClippingViewDelegate
- (void (^)(CGRect))picker_clippingViewWillBeginZooming:(TFY_ClippingView *)clippingView
{
    if (self.editedOperation == TFYEditingViewOperationNone && [self.clippingDelegate respondsToSelector:@selector(picker_EditingViewWillBeginEditing:)]) {
        [self.clippingDelegate picker_EditingViewWillBeginEditing:self];
    }
    self.editedOperation |= TFYEditingViewOperationZooming;
    ...
    void (^block)(CGRect) = ^(CGRect rect){
        if (clippingView.isReseting || clippingView.isRotating) { /** 重置/旋转 需要将遮罩显示也重置 */
            ...
            picker_me_dispatch_cancel(weakSelf.maskViewBlock);
        } else {
...
{
    if (self.editedOperation & TFYEditingViewOperationZooming) {
        self.editedOperation ^= TFYEditingViewOperationZooming;
    }
    __weak typeof(self) weakSelf = self;
    self.maskViewBlock = picker_dispatch_block_t(0.25f, ^{
        [weakSelf updateImagePixelText];
        
        if (weakSelf.editedOperation == TFYEditingViewOperationNone) {
            if (!weakSelf.gridView.isDragging) {
                if ([weakSelf.clippingDelegate respondsToSelector:@selector(picker_EditingViewDidEndEditing:)]) {
                    [weakSelf.clippingDelegate picker_EditingViewDidEndEditing:weakSelf];
                }
...
- (void)picker_clippingViewWillBeginDragging:(TFY_ClippingView *)clippingView
{
    ...
    self.editedOperation |= TFYEditingViewOperationDragging;
    /** 移动开始，隐藏 */
    self.gridView.showMaskLayer = NO;
    picker_me_dispatch_cancel(self.maskViewBlock);
}
- (void)picker_clippingViewDidEndDecelerating:(TFY_ClippingView *)clippingView
{
    ...
    if (!self.gridView.isDragging && !CGRectEqualToRect(self.gridView.gridRect, self.clippingView.frame)) {
        ...
        if (self.editedOperation & TFYEditingViewOperationDragging) {
            self.editedOperation ^= TFYEditingViewOperationDragging;
        }
    } else {
        if (self.editedOperation & TFYEditingViewOperationDragging) {
...
#pragma mark - TFYGridViewDelegate
- (void)picker_gridViewDidBeginResizing:(TFY_GridView *)gridView
{
    if (self.editedOperation == TFYEditingViewOperationNone && [self.clippingDelegate respondsToSelector:@selector(picker_EditingViewWillBeginEditing:)]) {
        [self.clippingDelegate picker_EditingViewWillBeginEditing:self];
    }
    self.editedOperation |= TFYEditingViewOperationGridResizing;
...
- (void)picker_gridViewDidEndResizing:(TFY_GridView *)gridView
{
    ...
    if (self.editedOperation & TFYEditingViewOperationGridResizing) {
        self.editedOperation ^= TFYEditingViewOperationGridResizing;
    }
...
- (void)refreshImageZoomViewCenter {
    CGFloat offsetX = (self.picker_width > self.contentSize.width) ? ((self.picker_width - self.contentSize.width) * 0.5) : 0.0;
    CGFloat offsetY = (self.picker_height > self.contentSize.height) ? ((self.picker_height - self.contentSize.height) * 0.5) : 0.0;
...
{
    /** 重置contentSize */
    CGRect realClipZoomRect = AVMakeRectWithAspectRatioInsideRect(self.clippingView.picker_size, self.clipZoomView.frame);
...
{
    /** 默认绘画线粗 */
    [self setDrawLineWidth:picker_editingView_drawLineWidth/self.zoomScale];
    /** 默认模糊线粗 */
    [self setSplashLineWidth:picker_editingView_splashWidth/self.zoomScale];
    /** 屏幕缩放率 */
...
- (NSDictionary *)photoEditData
{
    ...
    if (self.gridView.aspectRatio > 0 ) {
        NSDictionary *myData = @{kTFYEditingViewData_gridView_aspectRatio:@(self.gridView.aspectRatio)};
        [data setObject:myData forKey:kTFYEditingViewData];
    }
    
    NSDictionary *clippingViewData = self.clippingView.photoEditData;
    if (clippingViewData) [data setObject:clippingViewData forKey:kTFYEditingViewData_clippingView];
...
{
    self.clippingView.photoEditData = photoEditData[kTFYEditingViewData_clippingView];
    _clippingRect = self.clippingView.frame;
    self.gridView.angle = self.clippingView.angle;
    self.gridView.gridRect = self.clippingRect;
    self.maximumZoomScale = MIN(MAX(self.minimumZoomScale + self.defaultMaximumZoomScale - self.defaultMaximumZoomScale * (self.clippingView.zoomScale/self.clippingView.maximumZoomScale), self.minimumZoomScale), self.defaultMaximumZoomScale);
    NSDictionary *myData = photoEditData[kTFYEditingViewData];
    if (myData) {
        TFYGridViewAspectRatioType aspectRatio = [myData[kTFYEditingViewData_gridView_aspectRatio] integerValue];
