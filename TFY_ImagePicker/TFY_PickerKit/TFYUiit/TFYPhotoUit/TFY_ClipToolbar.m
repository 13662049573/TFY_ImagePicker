...
@implementation TFY_ClipToolbar
...
- (void)customInit
{
    ...
    int count = 4;
    BOOL canRotate = YES;
    if ([self.dataSource respondsToSelector:@selector(picker_clipToolbarCanRotate:)]) {
        canRotate = [self.dataSource picker_clipToolbarCanRotate:self];
        if (!canRotate) {
            count--;
        }
    }
    BOOL canAspectRatio = YES;
    if ([self.dataSource respondsToSelector:@selector(picker_clipToolbarCanAspectRatio:)]) {
        canAspectRatio = [self.dataSource picker_clipToolbarCanAspectRatio:self];
        if (!canAspectRatio) {
            count--;
        }
    }
...
- (void)clippingCancel:(UIButton *)button
{
    ...
    if ([self.delegate respondsToSelector:@selector(picker_clipToolbarDidCancel:)]) {
        [self.delegate picker_clipToolbarDidCancel:self];
    }
}
...
{
    _clickViewRect = button.frame;
    if ([self.delegate respondsToSelector:@selector(picker_clipToolbarDidReset:)]) {
        [self.delegate picker_clipToolbarDidReset:self];
    }
}
...
{
    _clickViewRect = button.frame;
    if ([self.delegate respondsToSelector:@selector(picker_clipToolbarDidRotate:)]) {
        [self.delegate picker_clipToolbarDidRotate:self];
    }
}
...
{
    _clickViewRect = button.frame;
    if ([self.delegate respondsToSelector:@selector(picker_clipToolbarDidAspectRatio:)]) {
        [self.delegate picker_clipToolbarDidAspectRatio:self];
    }
}
...
{
    _clickViewRect = button.frame;
    if ([self.delegate respondsToSelector:@selector(picker_clipToolbarDidFinish:)]) {
        [self.delegate picker_clipToolbarDidFinish:self];
    }
}
