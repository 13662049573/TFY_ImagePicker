...
#import "TFY_TextViewBackgroundLayoutManager.h"
...
@property (nonatomic, strong) TFY_TextViewBackgroundLayoutManager *layoutManager;

@property (nonatomic, weak) TFY_PickColorView *picker_colorSlider;
@property (nonatomic, weak) UIView *picker_keyboardBar;
...
- (BOOL)becomeFirstResponder
{
    ...
    return [self.picker_textView becomeFirstResponder];
}

- (BOOL)resignFirstResponder
{
    [super resignFirstResponder];
    return [self.picker_textView resignFirstResponder];
}
...
- (void)setShowText:(TFY_PickerText *)showText
{
    ...
    if (showText.attributedText.length > 0) {
        ...
        [self.picker_textView setAttributedText:showText.attributedText];
        UIColor *textColor = self.layoutManager.type == TFYCGContextDrawTextBackgroundTypeNone ? self.picker_textView.textColor : self.layoutManager.usedColor;
        [self setTextSliderColor:textColor];
    }
    UIView *alignmentView = [self.picker_keyboardBar viewWithTag:TFYTextBarAlignmentTag];
    for (UIButton *btn in alignmentView.subviews) {
        btn.selected = (btn.tag == self.picker_textView.textAlignment);
    }
    UIButton *fontBGButton = [self.picker_keyboardBar viewWithTag:TFYTextBarFontBGTag];
    fontBGButton.selected = !(self.layoutManager.type == TFYCGContextDrawTextBackgroundTypeNone);
}
...
- (void)configCustomNaviBar
{
    ...
    UIView *topbar = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.picker_width, topbarHeight)];
    ...
    UIButton *finishButton = [[UIButton alloc] initWithFrame:CGRectMake(self.picker_width - editOkWidth - margin, topSubViewY, editOkWidth, size)];
...
- (void)configTextView
{
    ...
    self.layoutManager = [TFY_TextViewBackgroundLayoutManager new];
    ...
    UITextView *textView = [[UITextView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(_topbar.frame), self.picker_width, self.picker_height-CGRectGetHeight(_topbar.frame)) textContainer:container];
    ...
    self.picker_textView = textView;
}

- (void)configKeyBoardBar
{
    CGFloat margin = isiPad ? 40.f : 10.f;
    UIView *keyboardBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.picker_height-margin, self.picker_width, 44)];
     ...
     */
    {
        ...
        view.tag = TFYTextBarAlignmentTag;
    ...
    fontBG.tag = TFYTextBarFontBGTag;
    ...
    TFY_PickColorView *_colorSlider = [[TFY_PickColorView alloc] initWithFrame:CGRectMake(sliderX + (maxSliderWidth-sliderWidth)/2, (CGRectGetHeight(keyboardBar.frame)-sliderHeight)/2, sliderWidth, sliderHeight) colors:kSliderColors];
//    _colorSlider.showColor = kSliderColors[0]; /** 白色 */
    ...
    self.picker_colorSlider = _colorSlider;
    
    [self addSubview:keyboardBar];
    self.picker_keyboardBar = keyboardBar;
}

/** 设置文字拾起器默认颜色 */
- (void)setTextColor:(UIColor *)textColor
{
    if (self.layoutManager.type == TFYCGContextDrawTextBackgroundTypeNone) {
        self.picker_textView.textColor = textColor;
        self.layoutManager.usedColor = textColor;
    } else {
        self.picker_textView.textColor = [textColor isEqual:[UIColor whiteColor]] ? [UIColor blackColor] : [UIColor whiteColor];
        self.layoutManager.usedColor = textColor;
    }
}
...
{
    self.picker_colorSlider.color = color;
    [self setTextColor:self.picker_colorSlider.color];
}

- (void)setTextSliderColorAtIndex:(NSUInteger)index
{
    self.picker_colorSlider.index = index;
    [self setTextColor:self.picker_colorSlider.color];
}
...
- (void)alignment_buttonClick:(UIButton *)button
{
    ...
    self.picker_textView.textAlignment = (NSTextAlignment)button.tag;
    if (self.layoutManager.type != TFYCGContextDrawTextBackgroundTypeNone) {
...
- (void)fontBG_buttonClick:(UIButton *)button
{
    ...
    if (button.isSelected) {
        self.layoutManager.type = TFYCGContextDrawTextBackgroundTypeSolid;
    } else {
        self.layoutManager.type = TFYCGContextDrawTextBackgroundTypeNone;
    }
    [self setTextColor:self.picker_colorSlider.color];
}
...
{
    if ([self.delegate respondsToSelector:@selector(picker_textBarControllerDidCancel:)]) {
        [self.delegate picker_textBarControllerDidCancel:self];
    }
}
...
- (void)finishButtonClick
{
    if ([self.delegate respondsToSelector:@selector(picker_textBarController:didFinishText:)]) {
        ...
        [self.delegate picker_textBarController:self didFinishText:text];
    }
}
...
- (void)keyboardWillShow:(NSNotification *)notification
{
    ...
    [UIView animateWithDuration:duration animations:^{
        self.picker_keyboardBar.picker_y = self.picker_height-CGRectGetHeight(keyboardRect)-CGRectGetHeight(self.picker_keyboardBar.frame);
        self.picker_textView.picker_height = self.picker_height-self.picker_keyboardBar.picker_y;
    }];
}
...
//    NSString *searchString = @"\n";
    ...
    else
    {
        ...
        if ([self.delegate respondsToSelector:@selector(picker_textBarControllerDidReachMaximumLimit:)]) {
            [self.delegate picker_textBarControllerDidReachMaximumLimit:self];
        }
        return NO;
    }
