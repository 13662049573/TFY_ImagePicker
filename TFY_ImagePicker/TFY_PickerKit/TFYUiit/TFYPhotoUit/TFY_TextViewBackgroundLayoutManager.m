//
//  TFY_TextViewBackgroundLayoutManager.m
//  WonderfulZhiKang
//
...
@interface TFY_TextViewBackgroundLayoutManager (){
    NSInteger maxIndex;
}
...
@implementation TFY_TextViewBackgroundLayoutManager
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (NSDictionary *)layoutData
{
    ...
    [data setObject:@(self.type) forKey:TFYCGContextDrawTextBackgroundTypeName];
    [data setObject:@(self.radius) forKey:TFYCGContextDrawTextBackgroundRadiusName];
    if (self.usedColor) {
        [data setObject:self.usedColor forKey:TFYCGContextDrawTextBackgroundColorName];
    }
    [data setObject:self.allUsedRects forKey:TFYCGContextDrawTextBackgroundLineUsedRectsName];
    [data setObject:[NSValue valueWithCGSize:self.textContainers.firstObject.size] forKey:TFYCGContextDrawTextBackgroundTextContainerSizeName];
    return data.copy;
}

- (void)setLayoutData:(NSDictionary *)layoutData
{
    self.usedColor = [layoutData objectForKey:TFYCGContextDrawTextBackgroundColorName];
    self.radius = [[layoutData objectForKey:TFYCGContextDrawTextBackgroundRadiusName] floatValue];
    self.type = [[layoutData objectForKey:TFYCGContextDrawTextBackgroundTypeName] integerValue];
}
...
//    NSLog(@"sqmTest:rect: %f,%f, %f,%f",rect.origin.x, rect.origin.y, rect.size.width, rect.size.height);
    ...
    picker_CGContextDrawTextBackground(context, self.usedColor, self.radius, self.rectArray, (TFYCGContextDrawTextBackgroundType)self.type);
