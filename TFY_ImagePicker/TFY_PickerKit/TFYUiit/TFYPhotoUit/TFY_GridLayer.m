...
NSString *const TFYgridLayerAnimtionKey = @"tfygridLayer_contentsRectAnimate";

@interface TFY_GridLayer ()<CAAnimationDelegate>
@property (nonatomic, copy) void (^callback)(BOOL finished);
@end

@implementation TFY_GridLayer
...
- (void)setGridRect:(CGRect)gridRect animated:(BOOL)animated completion:(void (^)(BOOL finished))completion
{
    ...
    if (animated) {
        ...
        self.path = path;
        [self addAnimation:animate forKey:TFYgridLayerAnimtionKey];
...
{
    if ([self animationForKey:TFYgridLayerAnimtionKey] == anim) {
        if (self.callback) {
            self.callback(flag);
            self.callback = nil;
        }
        [self removeAnimationForKey:TFYgridLayerAnimtionKey];
    }
}
