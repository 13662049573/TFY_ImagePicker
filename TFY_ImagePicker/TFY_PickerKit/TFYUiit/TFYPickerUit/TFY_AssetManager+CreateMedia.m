//
//  TFY_AssetManager+CreateMedia.m
//  WonderfulZhiKang
//
//  Created by 田风有 on 2022/12/21.
//

#import "TFY_AssetManager+CreateMedia.h"
#import <MobileCoreServices/UTCoreTypes.h>
#import "TFYCategory.h"

NSString *const CreateMediaFolder = @"TFY_AssetManager.CreateMedia";

@implementation TFY_AssetManager (CreateMedia)
...
- (NSData *)createGifDataWithImages:(NSArray <UIImage *>*)images size:(CGSize)size duration:(NSTimeInterval)duration loopCount:(NSUInteger)loopCount error:(NSError **)error
{
    ...
    for (UIImage *image in images) {
        UIImage *newImage = [image picker_scaleToSize:size];
...
- (UIImage *)createGifWithImages:(NSArray <UIImage *>*)images size:(CGSize)size duration:(NSTimeInterval)duration loopCount:(NSUInteger)loopCount error:(NSError **)error
{
    ...
    if (imageData) {
        image = [UIImage picker_imageWithImageData:imageData];
    }
    return image;
}
