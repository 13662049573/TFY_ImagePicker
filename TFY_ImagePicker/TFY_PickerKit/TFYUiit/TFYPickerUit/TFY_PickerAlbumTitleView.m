...
@interface TFY_PickerAlbumTitleView ()<UITableViewDataSource, UITableViewDelegate, UIGestureRecognizerDelegate>
...
@implementation TFY_PickerAlbumTitleView
    ...
    if (self.superview) {
        ...
        if (!self.selectedAlbum) {
            if (self.albumArr.count) {
                ...
                /** 重置状态 */
                _state = TFYAlbumTitleViewStateInactive;
...
- (void)setState:(TFYAlbumTitleViewState)state
{
    if (self.superview == nil) return;
    if (_state != state) {
        _state = state;
        switch (state) {
            case TFYAlbumTitleViewStateInactive:
            {
                [self hiddenMenu];
            }
                break;
                
            case TFYAlbumTitleViewStateActivity:
            {
                [self showMenu];
            }
                break;
...
- (void)setAlbumArr:(NSArray<TFY_PickerAlbum *> *)albumArr
{
    _albumArr = albumArr;
    if (self.superview) {
        if (self.selectedAlbum && ![self.albumArr containsObject:self.selectedAlbum]) {
            ...
            if (self.title) {
                ...
                TFY_PickerAlbum *album = self.albumArr.firstObject;
...
- (void)createMenuView
{
    ...
    [tableView registerClass:[TFY_PickerAlbumCell class] forCellReuseIdentifier:@"TFY_PickerAlbumCell"];
...
- (void)tapCall
{
    self.state = (self.state == TFYAlbumTitleViewStateActivity) ? TFYAlbumTitleViewStateInactive : TFYAlbumTitleViewStateActivity;
}

- (void)backgroundViewTapCall
{
    self.state = TFYAlbumTitleViewStateInactive;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    TFY_PickerAlbumCell *cell = [tableView dequeueReusableCellWithIdentifier:@"TFY_PickerAlbumCell"];
    
    TFY_PickerAlbum *album = _albumArr[indexPath.row];
    cell.album = album;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TFY_PickerAlbum *model = _albumArr[indexPath.row];
...
{
    return [TFY_PickerAlbumCell cellHeight];
}
