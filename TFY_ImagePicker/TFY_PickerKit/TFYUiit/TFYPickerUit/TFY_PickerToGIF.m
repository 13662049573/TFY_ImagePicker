...
#define tmp_fileName     @"_TFYNSGIF.gif"
#define tolerance    @(0.01)

@implementation TFY_PickerToGIF
...
+ (void)optimalGIFfromURL:(NSURL*)videoURL loopCount:(int)loopCount completion:(void(^)(NSURL *GifURL))completionBlock {
    ...
    TFYGIFSize optimalSize = TFYGIFSizeMedium;
    if (videoWidth >= 1200 || videoHeight >= 1200)
        optimalSize = TFYGIFSizeVeryLow;
    else if (videoWidth >= 800 || videoHeight >= 800)
        optimalSize = TFYGIFSizeLow;
    else if (videoWidth >= 400 || videoHeight >= 400)
        optimalSize = TFYGIFSizeMedium;
    else if (videoWidth < 400|| videoHeight < 400)
        optimalSize = TFYGIFSizeHigh;
    
    [self createGIFfromURL:videoURL delayTime:0 loopCount:loopCount TFYGIFSize:optimalSize completion:completionBlock];
...
+ (void)createGIFfromURL:(NSURL*)videoURL delayTime:(NSTimeInterval)delayTime loopCount:(NSUInteger)loopCount TFYGIFSize:(TFYGIFSize)TFYGIFSize completion:(void(^)(NSURL *GifURL))completionBlock {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        gifURL = [self createGIFforTimePoints:timePoints fromURL:videoURL fileProperties:fileProperties frameProperties:frameProperties frameCount:frameCount TFYGIFSize:TFYGIFSize];
...
+ (NSURL *)createGIFforTimePoints:(NSArray *)timePoints fromURL:(NSURL *)url fileProperties:(NSDictionary *)fileProperties frameProperties:(NSDictionary *)frameProperties frameCount:(int)frameCount TFYGIFSize:(TFYGIFSize)TFYGIFSize{
    ...
    for (NSValue *time in timePoints) {
        ...
        imageRef = (float)TFYGIFSize/10 != 1 ? ImageWithScale([generator copyCGImageAtTime:[time CMTimeValue] actualTime:nil error:&error], (float)TFYGIFSize/10) : [generator copyCGImageAtTime:[time CMTimeValue] actualTime:nil error:&error];
#elif TARGET_OS_MAC
