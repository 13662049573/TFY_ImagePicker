...
NSString *const kTFYDrawViewData = @"TFYDrawViewData";

@interface TFY_DrawView ()
{
    BOOL _isWork;
    BOOL _isBegan;
}
    ...
    if ([event allTouches].count == 1 && self.brush) {
        ...
        if (layer) {
            ...
            [self.layer.sublayers enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(__kindof CALayer * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                /** 图层层级<=，放在该图层上方 */
                if (layer.picker_level <= obj.picker_level) {
...
- (NSDictionary *)data
{
    if (self.brushData.count) {
        return @{kTFYDrawViewData:[self.brushData copy]};
    }
    return nil;
}
...
{
    NSArray *brushData = data[kTFYDrawViewData];
    if (brushData.count) {
        for (NSDictionary *allTracks in brushData) {
            CALayer *layer = [TFY_Brush drawLayerWithTrackDict:allTracks];
            if (layer) {
                /** 使用画笔的图层层级，层级越大，图层越低 */
                [self.layer.sublayers enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(__kindof CALayer * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    /** 图层层级<=，放在该图层上方 */
