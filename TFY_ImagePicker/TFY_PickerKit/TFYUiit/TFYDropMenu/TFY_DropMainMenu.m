...
@property (nonatomic, strong) NSMutableArray<id <TFY_DropItemProtocol>> *m_items;
...
@implementation TFY_DropMainMenu
...
- (NSArray<id<TFY_DropItemProtocol>> *)items
{
...
{
    if (direction == TFYDropMainMenuDirectionAuto) {
        direction = TFYDropMainMenuDirectionBottom;
    }
...
- (void)dismissWithAnimated:(BOOL)animated
{
    if (animated) {
        ...
        switch (self.direction) {
            case TFYDropMainMenuDirectionTop:
            {
...
- (void)_customInit
{
    ...
    _direction = TFYDropMainMenuDirectionBottom;
...
- (void)_createContainView
{
    ...
    [self.MyCollectView registerClass:[TFY_BaseCollectionViewCell class] forCellWithReuseIdentifier:[TFY_BaseCollectionViewCell identifier]];
...
- (void)_drawCircleView
{
    ...
    CGFloat startY = 0.f;
    if (self.direction == TFYDropMainMenuDirectionBottom) {
...
- (CGRect)_calculateMenuViewFrameWithConverFrame:(CGRect)converFrame
{
    ...
    for (NSUInteger i = 0; i < self.displayMaxNum; i ++) {
            id<TFY_DropItemProtocol>obj = [self.items objectAtIndex:i];
    ...
    if (self.direction == TFYDropMainMenuDirectionTop || self.direction == TFYDropMainMenuDirectionBottom) {
...
- (void)_calculateItemViews
{
    ...
    for (id<TFY_DropItemProtocol>obj in self.m_items) {
...
- (void)_showFromFrame:(CGRect)frame animated:(BOOL)animated
{
    ...
    } else if (self.direction == TFYDropMainMenuDirectionBottom) {
    ...
    if (animated) {
        ...
        switch (self.direction) {
            case TFYDropMainMenuDirectionTop:
            {
                ...
                break;
            case TFYDropMainMenuDirectionBottom:
            {
                zeroRect.size.height = 0.f;
            }
                break;
...
{
    id<TFY_DropItemProtocol>obj = [self.items objectAtIndex:indexPath.row];
...
{
    for (id<TFY_DropItemProtocol>obj in self.items) {
        obj.selected = NO;
    }
    {
        id<TFY_DropItemProtocol>obj = [self.items objectAtIndex:indexPath.row];
...
{
    NSString *identifier = [TFY_BaseCollectionViewCell identifier];
    TFY_BaseCollectionViewCell *cell = (TFY_BaseCollectionViewCell *)[collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:indexPath];
