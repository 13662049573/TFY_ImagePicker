...
@implementation TFY_ImageUtil

static CGContextRef picker_CreateRGBABitmapContext (CGImageRef inImage)// 返回一个使用RGBA通道的位图上下文
{
...
static unsigned char *picker_RequestImagePixelData(UIImage *inImage)
// 返回一个指针，该指针指向一个数组，数组中的每四个元素都是图像上的一个像素点的RGBA的数值(0-255)，用无符号的char是因为它正好的取值范围就是0-255
{
    CGImageRef img = [inImage CGImage];
    CGSize size = [inImage size];
    
    CGContextRef cgctx = picker_CreateRGBABitmapContext(img); //使用上面的函数创建上下文
...
static void picker_changeRGBA(int *red,int *green,int *blue,int *alpha, const float* f)//修改RGB的值
{
    int redV = *red;
    int greenV = *green;
    int blueV = *blue;
...
+ (UIImage*)picker_imageWithImage:(UIImage*)inImage withColorMatrix:(const float*) f
{
    ...
    unsigned char *imgPixel = picker_RequestImagePixelData(inImage);
    ...
    for(GLuint y = 0;y< h;y++)//双层循环按照长宽的像素个数迭代每个像素点
    {
        ...
        for (GLuint x = 0; x<w; x++)
        {
            ...
            picker_changeRGBA(&red, &green, &blue, &alpha, f);
