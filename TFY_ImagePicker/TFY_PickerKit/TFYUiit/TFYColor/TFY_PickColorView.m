...
CGFloat const TFYPickColorView_Default_Height = 40.0f; //默认高度

CGFloat const TFYPickColorView_Default_ColorMaxWidth = 15.0f; // 默认最大宽度

CGFloat const TFYPickColorView_Default_ColorMinWidth = 10.0f; // 默认最小宽度

CGFloat const TFYPickColorView_magnifierView_WitdhOrHeight = 25.0f; //!放大镜大小

CGFloat const TFYPickColorView_magnifierView_Margin = 15.0f; //!放大镜距离滑块间距

@interface TFY_PickColorView ()<UIGestureRecognizerDelegate>
...
@implementation TFY_PickColorView
...
- (void)setColor:(UIColor *)color{
    ...
    for (NSInteger i=0; i<self.colors.count; i++) {
        ...
        if ([color picker_isEqualToColor:color1]) {
            currentIndex = i;
            isYES = YES;
            break;
        }
...
- (void)commUI{
    ...
    if (!_showColorWindow) {
        _showColorWindow = TFYAppWindow();
    }
    if (!_magnifierView) {
        UIView *customView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, TFYPickColorView_magnifierView_WitdhOrHeight, TFYPickColorView_magnifierView_WitdhOrHeight)];
        customView.hidden = YES;
        customView.center = CGPointMake(_showColorsContainer.center.x, _showColorsContainer.center.y - 40);
        customView.backgroundColor = [UIColor clearColor];
        customView.layer.cornerRadius = TFYPickColorView_magnifierView_WitdhOrHeight/2;
...
- (void)calu:(NSInteger)gestureRecognizer point:(CGPoint)point comple:(void(^)(UIColor *color, CGPoint center))comple{
    ...
    if ([self.delegate respondsToSelector:@selector(picker_PickColorView:didSelectColor:)]) {
        [self.delegate picker_PickColorView:self didSelectColor:changeColor];
    }
...
//    }
    ...
    [self calu:0 point:_initialPoint comple:^(UIColor *color, CGPoint center) {
        ...
        point.y -= TFYPickColorView_magnifierView_Margin + weakSelf.magnifierView.frame.size.height/2 + weakSelf.showColorsContainer.frame.size.height/2;
...
- (void)createShowColorsContainer{
    ...
    showColorsContainerW = MAX(showColorsContainerW, TFYPickColorView_Default_ColorMinWidth);
    showColorsContainerW = MIN(showColorsContainerW, TFYPickColorView_Default_ColorMaxWidth);
    CGRect rect = CGRectMake((_colorWidth - showColorsContainerW) / 2, (CGRectGetHeight(self.frame) - TFYPickColorView_Default_ColorHeight * 2.5)/2, showColorsContainerW, TFYPickColorView_Default_ColorHeight * 2.5);
...
- (void)createRectangleView{
    for (NSInteger i = 0; i < self.colors.count; i ++) {
        ...
        CGRect rect = CGRectMake(x, (CGRectGetHeight(self.frame) - TFYPickColorView_Default_ColorHeight)/2, _colorWidth, TFYPickColorView_Default_ColorHeight);
        NSInteger line = 4;
        if (i == 0) {
            line = 1;
        ...
        CGFloat cornerRadius = TFYPickColorView_Default_ColorHeight / 2;
