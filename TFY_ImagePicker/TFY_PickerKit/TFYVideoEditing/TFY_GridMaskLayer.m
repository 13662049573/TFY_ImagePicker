...
@implementation TFY_GridMaskLayer
...
- (void)setMaskRect:(CGRect)maskRect animated:(BOOL)animated
{
    ...
    [self removeAnimationForKey:@"picker_maskLayer_opacityAnimate"];
    if (animated) {
        ...
        animate.toValue = @(1.0);
        self.path = path;
        [self addAnimation:animate forKey:@"picker_maskLayer_opacityAnimate"];
    } else {
        self.path = path;
    }
    CGPathRelease(path);
